...
class Solve:  # noqa: PLR0904
    ...
    def ghost_splits(
        ...
        """
        Map each tracked step to its cumulative completion time in ns.

        The split of a step is the time of its last move which
        mirrors how the live stopwatch measures ``elapsed_time`` from the
        solve start rather than summing per-step totals. Steps are keyed
        by name so alignment with the live checkpoints is by identity, not
        ordinal. A skipped step (present in the summary with empty
        ``times``) inherits the previous checkpoint time, so its group
        completes "instantly". A tracked step the ghost's summary does not
        name at all (e.g. an ``XXXXCross`` ghost against a tracked
        ``Cross``) is omitted, so that live checkpoint simply races without
        a delta rather than against a bogus one.
        ...
        Returns:
            Mapping of tracked step name to cumulative ns, empty when the
            solve carries no reconstruction.
