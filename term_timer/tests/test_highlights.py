...
from term_timer.highlights import get_auf_highlight
from term_timer.highlights import get_cross_highlight
from term_timer.highlights import get_fluency_highlight
from term_timer.highlights import get_missed_moves_highlight
from term_timer.highlights import get_no_pauses_highlight
from term_timer.highlights import get_optimal_f2l_highlight
from term_timer.highlights import get_optimal_ll_highlight
from term_timer.highlights import get_recognition_highlight
from term_timer.highlights import get_score_highlight
from term_timer.highlights import get_skip_highlight
from term_timer.highlights import get_step_recognition_highlight
from term_timer.highlights import get_time_highlight
...
class TestGetCrossHighlight(unittest.TestCase):
    """Tests for get_cross_highlight function."""
    ...
    def test_optimal_cross_4_htm(self) -> None:
        """Test optimal cross highlight for exactly 4 HTM."""
        summary = [self.create_step_summary('Cross', 4)]
        result = get_cross_highlight(summary)
        self.assertEqual(result, 'Optimal cross in 4 HTM!')

    def test_optimal_cross_3_htm(self) -> None:
        """Test optimal cross highlight for less than 4 HTM."""
    ...
    def test_optimal_cross_1_htm(self) -> None:
        """Test optimal cross highlight for 1 HTM."""
    ...
    def test_efficient_cross_5_htm(self) -> None:
        """Test efficient cross highlight for 5 HTM."""
    ...
    def test_efficient_cross_6_htm(self) -> None:
        """Test efficient cross highlight for exactly 6 HTM."""
        summary = [self.create_step_summary('Cross', 6)]
        result = get_cross_highlight(summary)
        self.assertEqual(result, 'Efficient cross in 6 HTM.')

    def test_no_highlight_for_7_htm(self) -> None:
        """Test no highlight returned for cross with more than 6 HTM."""
        summary = [self.create_step_summary('Cross', 7)]
        result = get_cross_highlight(summary)
        self.assertEqual(result, '')

    def test_no_highlight_for_10_htm(self) -> None:
        """Test no highlight returned for inefficient cross."""
        summary = [self.create_step_summary('Cross', 10)]
        result = get_cross_highlight(summary)
        self.assertEqual(result, '')

    def test_no_cross_in_summary(self) -> None:
        """Test no highlight when Cross step is not in summary."""
        summary = [self.create_step_summary('F2L 1', 5)]
        result = get_cross_highlight(summary)
        self.assertEqual(result, '')

    def test_empty_summary(self) -> None:
        """Test no highlight for empty summary list."""
        summary: list[StepSummary] = []
        result = get_cross_highlight(summary)
        self.assertEqual(result, '')


class TestGetXcrossHighlight(unittest.TestCase):
    """Tests for get_xcross_highlight function."""
    ...
    def test_no_xcross(self) -> None:
        """Test no highlight when XCross is not in summary."""
        summary = [
    ...
    def test_empty_summary(self) -> None:
        """Test no highlight for empty summary."""
        summary: list[StepSummary] = []
        result = get_xcross_highlight(summary)
        self.assertEqual(result, '')

    def test_case_sensitive_xcross(self) -> None:
        """Test that lowercase xcross does not trigger highlight."""
        summary = [self.create_step_summary('xcross')]
        result = get_xcross_highlight(summary)
        self.assertEqual(result, '')


class TestGetMissedMovesHighlight(unittest.TestCase):
    """Tests for get_missed_moves_highlight function."""

    def test_no_missed_moves(self) -> None:
        """Test highlight when there are no missed moves."""
        solve = Mock()
        solve.all_missed_moves = 0
        result = get_missed_moves_highlight(solve)
        self.assertEqual(result, 'Perfect execution - no wasted moves!')

    def test_with_missed_moves(self) -> None:
        """Test no highlight when there are missed moves."""
        solve = Mock()
    ...
    def test_with_one_missed_move(self) -> None:
        """Test no highlight for single missed move."""
        solve = Mock()
    ...
    def test_with_many_missed_moves(self) -> None:
        """Test no highlight for many missed moves."""
        solve = Mock()
...
class TestGetScoreHighlight(unittest.TestCase):
    """Tests for get_score_highlight function."""

    def test_score_exactly_16(self) -> None:
        """Test highlight when score is exactly 16."""
        solve = Mock()
        solve.score = 16.0
        result = get_score_highlight(solve)
        self.assertEqual(result, 'Excellent solve score of 16.00.')

    def test_score_above_16(self) -> None:
        """Test highlight when score is above 16."""
        solve = Mock()
        solve.score = 18.5
        result = get_score_highlight(solve)
    ...
    def test_score_maximum(self) -> None:
        """Test highlight with maximum score."""
        solve = Mock()
        solve.score = 20.0
        result = get_score_highlight(solve)
    ...
    def test_score_below_16(self) -> None:
        """Test no highlight when score is below 16."""
        solve = Mock()
        solve.score = 15.99
        result = get_score_highlight(solve)
        self.assertEqual(result, '')
    ...
    def test_score_zero(self) -> None:
        """Test no highlight for zero score."""
        solve = Mock()
        solve.score = 0.0
        result = get_score_highlight(solve)
        self.assertEqual(result, '')
    ...
    def test_score_negative(self) -> None:
        """Test no highlight for negative score."""
        solve = Mock()
        solve.score = -5.0
        result = get_score_highlight(solve)
        self.assertEqual(result, '')
...
class TestGetTimeHighlight(unittest.TestCase):
    """Tests for get_time_highlight function."""

    def test_time_10_seconds(self) -> None:
        """Test time highlight for 10 second solve."""
        solve = Mock()
        solve.time = 10 * SECOND
        result = get_time_highlight(solve)
        self.assertEqual(result, 'Keep pushing for sub-9.5.')

    def test_time_20_seconds(self) -> None:
        """Test time highlight for 20 second solve."""
        solve = Mock()
    ...
    def test_time_one_second(self) -> None:
        """Test time highlight for 1 second solve."""
        solve = Mock()
        solve.time = 1 * SECOND
        result = get_time_highlight(solve)
    ...
    def test_time_with_decimals(self) -> None:
        """Test time highlight with decimal seconds."""
        solve = Mock()
    ...
    def test_time_very_fast(self) -> None:
        """Test time highlight for very fast solve."""
        solve = Mock()
    ...
    def test_time_always_returns_message(self) -> None:
        """Test that time highlight always returns a message."""
        solve = Mock()
...
class TestGetFluencyHighlight(unittest.TestCase):
    """Tests for get_fluency_highlight function."""

    def test_fluency_exactly_90(self) -> None:
        """Test highlight for fluency of exactly 90."""
        solve = Mock()
        solve.fluency = 90
        result = get_fluency_highlight(solve)
        self.assertEqual(result, 'Buttery smooth! 90/100 fluency.')

    def test_fluency_above_90(self) -> None:
        """Test highlight for fluency above 90."""
        solve = Mock()
        solve.fluency = 95
        result = get_fluency_highlight(solve)
    ...
    def test_fluency_100(self) -> None:
        """Test highlight for perfect fluency."""
        solve = Mock()
        solve.fluency = 100
        result = get_fluency_highlight(solve)
    ...
    def test_fluency_exactly_80(self) -> None:
        """Test highlight for fluency of exactly 80."""
        solve = Mock()
        solve.fluency = 80
        result = get_fluency_highlight(solve)
    ...
    def test_fluency_between_80_and_90(self) -> None:
        """Test highlight for fluency between 80 and 90."""
        solve = Mock()
        solve.fluency = 85
        result = get_fluency_highlight(solve)
    ...
    def test_fluency_89(self) -> None:
        """Test highlight for fluency just below 90."""
        solve = Mock()
        solve.fluency = 89
        result = get_fluency_highlight(solve)
    ...
    def test_fluency_below_80(self) -> None:
        """Test no highlight for fluency below 80."""
        solve = Mock()
        solve.fluency = 79
        result = get_fluency_highlight(solve)
        self.assertEqual(result, '')
    ...
    def test_fluency_zero(self) -> None:
        """Test no highlight for zero fluency."""
        solve = Mock()
        solve.fluency = 0
        result = get_fluency_highlight(solve)
        self.assertEqual(result, '')
    ...
    def test_fluency_low(self) -> None:
        """Test no highlight for low fluency."""
        solve = Mock()
        solve.fluency = 50
        result = get_fluency_highlight(solve)
        self.assertEqual(result, '')
...
class TestGetTpsHighlight(unittest.TestCase):
    """Tests for get_tps_highlight function."""

    def test_tps_exactly_5(self) -> None:
        """Test highlight for TPS of exactly 5.0."""
        solve = Mock()
        solve.tps = 5.0
        result = get_tps_highlight(solve)
        self.assertEqual(result, 'Lightning fingers! 5.0 TPS.')

    def test_tps_above_5(self) -> None:
        """Test highlight for TPS above 5.0."""
        solve = Mock()
        solve.tps = 6.5
        result = get_tps_highlight(solve)
    ...
    def test_tps_very_high(self) -> None:
        """Test highlight for very high TPS."""
        solve = Mock()
        solve.tps = 10.0
        result = get_tps_highlight(solve)
    ...
    def test_tps_exactly_4_5(self) -> None:
        """Test highlight for TPS of exactly 4.5."""
        solve = Mock()
        solve.tps = 4.5
        result = get_tps_highlight(solve)
    ...
    def test_tps_between_4_5_and_5(self) -> None:
        """Test highlight for TPS between 4.5 and 5.0."""
        solve = Mock()
        solve.tps = 4.8
        result = get_tps_highlight(solve)
    ...
    def test_tps_below_4_5(self) -> None:
        """Test no highlight for TPS below 4.5."""
        solve = Mock()
        solve.tps = 4.4
        result = get_tps_highlight(solve)
        self.assertEqual(result, '')
    ...
    def test_tps_zero(self) -> None:
        """Test no highlight for zero TPS."""
        solve = Mock()
        solve.tps = 0.0
        result = get_tps_highlight(solve)
        self.assertEqual(result, '')
...
class TestGetNoPausesHighlight(unittest.TestCase):
    """Tests for get_no_pauses_highlight function."""

    def test_zero_pauses(self) -> None:
        """Test highlight for zero pauses."""
        solve = Mock()
        solve.execution_pauses = 0
        result = get_no_pauses_highlight(solve)
        self.assertEqual(result, 'Flawless lookahead - zero pauses!')

    def test_one_pause(self) -> None:
        """Test highlight for one pause."""
        solve = Mock()
    ...
    def test_two_pauses(self) -> None:
        """Test highlight for exactly two pauses."""
        solve = Mock()
    ...
    def test_three_pauses(self) -> None:
        """Test no highlight for three pauses."""
        solve = Mock()
    ...
    def test_many_pauses(self) -> None:
        """Test no highlight for many pauses."""
        solve = Mock()
...
class TestGetSkipHighlight(unittest.TestCase):
    """Tests for get_skip_highlight function."""
    ...
    def test_pll_skip_only(self) -> None:
        """Test highlight for PLL skip only."""
        summary = [
    ...
    def test_no_skips(self) -> None:
        """Test no highlight when there are no skips."""
        summary = [
            self.create_step_summary('OLL', 'step'),
            self.create_step_summary('PLL', 'step'),
        ]
...
class TestGetRecognitionHighlight(unittest.TestCase):
    """Tests for get_recognition_highlight function."""
...
class TestGetAufHighlight(unittest.TestCase):
    """Tests for get_auf_highlight function."""
...
class TestIsOptimalLlStep(unittest.TestCase):
    ...
    @patch('term_timer.highlights.get_case')
...
class TestGetOptimalLlHighlight(unittest.TestCase):
    """Tests for get_optimal_ll_highlight function."""
    ...
    def test_only_pll_optimal(self, mock_is_optimal: Mock) -> None:
        """Test highlight for only PLL optimal."""
        def is_optimal_side_effect(step: 'StepSummary') -> bool:
    ...
    def test_neither_optimal(self, mock_is_optimal: Mock) -> None:
        """Test no highlight when neither OLL nor PLL is optimal."""
    ...
    def test_only_f2l_steps(self, mock_is_optimal: Mock) -> None:
        """Test no highlight when only F2L steps are present."""
        summary = [
            self.create_step_summary('F2L 1'),
...
class TestGetOptimalF2lHighlight(unittest.TestCase):
    """Tests for get_optimal_f2l_highlight function."""
    ...
    def test_all_four_f2l_pairs_optimal(self, mock_get_case: Mock) -> None:
        """Test highlight when all 4 F2L pairs are optimal."""
        mock_case = Mock()
        mock_case.optimal_htm = 10
        mock_get_case.return_value = mock_case
    ...
    def test_two_f2l_pairs_optimal(self, mock_get_case: Mock) -> None:
        """Test highlight when 2 F2L pairs are optimal."""
        mock_case = Mock()
        mock_case.optimal_htm = 10
        mock_get_case.return_value = mock_case
    ...
    def test_one_f2l_pair_optimal(self, mock_get_case: Mock) -> None:
        """Test no highlight when only 1 F2L pair is optimal."""
        mock_case = Mock()
        mock_case.optimal_htm = 10
        mock_get_case.return_value = mock_case
    ...
    def test_no_f2l_pairs_optimal(self, mock_get_case: Mock) -> None:
        """Test no highlight when no F2L pairs are optimal."""
        mock_case = Mock()
        mock_case.optimal_htm = 10
        mock_get_case.return_value = mock_case
...
class TestGetStepRecognitionHighlight(unittest.TestCase):
    """Tests for get_step_recognition_highlight function."""
    ...
    def test_recognition_exactly_10_percent(self) -> None:
        """Test no highlight for recognition of exactly 10%."""
        summary = [
    ...
    def test_recognition_above_10_percent(self) -> None:
        """Test no highlight for recognition above 10%."""
        summary = [
    ...
    def test_recognition_zero_percent(self) -> None:
        """Test highlight for zero percent recognition."""
    ...
    def test_first_matching_step_wins(self) -> None:
        """Test that only first matching step generates highlight."""
        summary = [
...
class TestGenerateSolveHighlights(unittest.TestCase):
    """Tests for generate_solve_highlights function."""
    ...
    def test_no_method_applied_returns_empty(self) -> None:
        ...
        result = generate_solve_highlights(solve)
        self.assertEqual(result, [])

    @patch('term_timer.highlights.get_cross_highlight')
    @patch('term_timer.highlights.get_xcross_highlight')
    @patch('term_timer.highlights.get_skip_highlight')
    @patch('term_timer.highlights.get_optimal_f2l_highlight')
    @patch('term_timer.highlights.get_optimal_ll_highlight')
    @patch('term_timer.highlights.get_missed_moves_highlight')
    @patch('term_timer.highlights.get_fluency_highlight')
    @patch('term_timer.highlights.get_tps_highlight')
    @patch('term_timer.highlights.get_no_pauses_highlight')
    @patch('term_timer.highlights.get_recognition_highlight')
    @patch('term_timer.highlights.get_step_recognition_highlight')
    @patch('term_timer.highlights.get_auf_highlight')
    @patch('term_timer.highlights.get_score_highlight')
    @patch('term_timer.highlights.get_time_highlight')
    def test_all_highlights_called(
        self,
        mock_time: Mock,
        mock_score: Mock,
        mock_auf: Mock,
        ...
        """Test that all highlight functions are called."""
        solve = self.create_mock_solve()
        mock_time.return_value = 'time highlight'

        generate_solve_highlights(solve)
    ...
    @patch('term_timer.highlights.get_score_highlight')
    @patch('term_timer.highlights.get_time_highlight')
    def test_filters_empty_highlights(
        self,
        mock_time: Mock,
        mock_score: Mock,
        mock_auf: Mock,
        ...
        mock_cross.return_value = 'Cross highlight'
        mock_xcross.return_value = ''
        mock_skip.return_value = 'Skip highlight'
        mock_opt_f2l.return_value = ''
        mock_opt_ll.return_value = ''
        mock_missed.return_value = 'Missed highlight'
        mock_fluency.return_value = ''
        mock_tps.return_value = 'TPS highlight'
    ...
    def test_returns_only_time_when_all_else_empty(
        ...
        """Test that at least time highlight is returned."""
        solve = self.create_mock_solve()
